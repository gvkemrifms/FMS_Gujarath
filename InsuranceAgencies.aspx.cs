...
{
    public IFleetMaster ObjInsAgency = new FMSFleetMaster();
    ...
    protected void btnInsuranceUpdate_Click(object sender, EventArgs e)
    {
        ...
        {
            case "Insert":
                if (ObjInsAgency.ICheckInsuranceAgency(txtInsuranceAgency.Text) > 0)
                ...
                break;
            default:
                if (ObjInsAgency.ICheckInsuranceAgency(int.Parse(ViewState["InsuranceId"].ToString()), txtInsuranceAgency.Text) <= 0)
                    result = ObjInsAgency.IUpdateInsuranceAgency(Convert.ToInt32(ViewState["InsuranceId"]), txtInsuranceAgency.Text, txtAddress.Text, txtContactPerson.Text, Convert.ToInt64(txtContactNo.Text));
                else
                    Show("Insurance Agency Already Exist");
                if (result > 0)
                {
    ...
    protected void grvInsuranceAgencyDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (e.CommandName)
        {
            case "EditAgency":
                ...
                break;
            case "DeleteAgency":
                ObjInsAgency.IDeleteInsuranceAgency(int.Parse(e.CommandArgument.ToString()));
